...
3. The first model assumes a compound symmetry covariance matrix, and the second a factor analysis (FAk). k = 2 was ad-hoc assigned to the models, i.e., no model selection performed given this is just an example. Both models included the genomic relationship (GBLUP, yielding genomic estimated breeding values within environments).
